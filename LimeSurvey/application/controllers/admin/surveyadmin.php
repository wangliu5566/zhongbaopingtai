...
class SurveyAdmin extends Survey_Common_Action
{
    ...
    */
    public function newsurvey()
    {
        ...
        $arrayed_data['data']['esrow']['format'] = 'S';
    ...
    public function editlocalsettings($iSurveyID)
    {
        ...
        if (Permission::model()->hasSurveyPermission($iSurveyID, 'surveylocale', 'read') || Permission::model()->hasSurveyPermission($iSurveyID, 'surveysettings', 'read'))
        {
            ...
            foreach ($grplangs as $i => $sLang)
            {
                ...
                $esrow['sDateFormatDefault'] = $esrow['surveyls_dateformat'];
                $esrow['aDateFormatData']=$aDateFormatData;
                $esrow['owner'] = $owner;
                $esrow['esrow'] = $surveyinfo;
                $esrow['esrow']['owner_id'] = $owner['uid'];
                $aData['dateformatdetails'] = $esrow['dateformatdetails'] = getDateFormatData(Yii::app()->session['dateformat']);

                Yii::app()->loadLibrary('Date_Time_Converter');
                $datetimeobj = new Date_Time_Converter($surveyinfo['expires'], "Y-m-d H:i:s");
                $esrow['expires'] = $datetimeobj->convert($esrow['dateformatdetails']['phpdate'].' H:i');

                $datetimeobj = new Date_Time_Converter($surveyinfo['startdate'], "Y-m-d H:i:s");
                $esrow['startdate'] = $datetimeobj->convert($esrow['dateformatdetails']['phpdate'].' H:i');
...
//        Yii::app()->session['flashmessage'] = gT("The new question group/question order was successfully saved.");
        Yii::app()->session['flashmessage'] = '新题序已成功保存。';
    }
    ...
    */
    function insert($iSurveyID=null)
    {
        if (Permission::model()->hasGlobalPermission('surveys','create'))
        {
            ...
            $oGroup->sid = $iNewSurveyid;
            $oGroup->group_name = 'A1';
            $oGroup->description = '';
            if(!isset($newGroupOrder)){
                $newGroupOrder = getMaxGroupOrder($iNewSurveyid);
            }
            $oGroup->group_order = $newGroupOrder;
            $oGroup->language = Yii::app()->request->getPost('language');
            $oGroup->randomization_group = '';
