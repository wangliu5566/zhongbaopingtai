...
class Authentication extends Survey_Common_Action
{
    ...
    public function csLogin(){
            $curl = curl_init();
            //设置抓取的url
            curl_setopt($curl, CURLOPT_URL, 'http://www.answer.com/index.php/admin/authentication/sa/login');
            //设置头文件的信息作为数据流输出
        ...
        if (User::model()->find("users_name=:users_name",array(':users_name'=>$new_user))) {
            ...
            if($res){
              ...
              /*  Permission::model()->insertSomeRecords([
                   'uid' => $id, 'permission' => Yii::app()->getConfig("defaulttemplate"), 'entity'=>'template', 'read_p' => 1, 'entity_id'=>0]);*/
                Permission::model()->saveBasicPermission($id);
                return true;
            }else{
                return false;
            }
    ...
    public function index()
    {
        ...
        // If not ajax, redirect to admin startpage or again to login form
            ...
            if ($succeeded) {
                if($survey){
                    App()->getController()->redirect(array('/index.php/admin/statistics/sa/simpleStatistics/surveyid'.$survey));
                }else{
                    self::doRedirect();
                }
    ...
    private function _redirectIfLoggedIn()
    {
        if (!Yii::app()->user->getIsGuest())
        {
            $survey = $_GET['survey'] ?? '';
            if($survey){
                App()->getController()->redirect(array('/admin/statistics/sa/simpleStatistics/surveyid/'.$survey));
            }else{
    ...
    {
        $returnUrl = App()->user->getReturnUrl(array('/admin/survey/sa/listsurveys'));
