...
class index extends CAction {
    ...
    public function run()
    {
        ...
        if(isset($getData['code']) && isset($getData['activityId'])){
           $_SESSION['saveActivityData']['code'] = $getData['code'];
           $_SESSION['saveActivityData']['activityId'] = $getData['activityId'];
                $ch = curl_init();
                //设置选项，包括URL
                curl_setopt($ch, CURLOPT_URL,
                    "http://crowdsourcing.jqweike.com/v1/frontend/activity/juade-answer?code=".$_SESSION['saveActivityData']['code']."&activityId=".$_SESSION['saveActivityData']['activityId']);
                ...
                //打印获得的数据
                if(!$output->data){
                    echo "<h1>你好,你没有权限查看本调查,请联系管理员</h1>";exit;
                }

        }else{
                if(isset($_SESSION['saveActivityData']['code']) && isset($_SESSION['saveActivityData']['activityId'])){
                    $ch = curl_init();
                    //设置选项，包括URL
                    curl_setopt($ch, CURLOPT_URL,
                        "http://crowdsourcing.jqweike.com/v1/frontend/activity/juade-answer?code=".$_SESSION['saveActivityData']['code']."&activityId=".$_SESSION['saveActivityData']['activityId']);
