...
class Question extends LSActiveRecord
{
    ...
    * @return void
    */
    function updateQuestionOrder($gid,$language,$position=1)
    {
    ...
    */
    ...
    public function getbuttons()
    {
        ...
        $button      = '<a class="btn btn-default open-preview"  data-toggle="tooltip" title="'.gT("Question preview").'"  aria-data-url="'.$previewUrl.'" aria-data-sid="'.$this->sid.'" aria-data-gid="'.$this->gid.'" aria-data-qid="'.$this->qid.'" aria-data-language="'.$this->language.'" role="button" ><span class="glyphicon glyphicon-eye-open"  ></span></a> ';
        ...
        if($oSurvey->active != "Y" && Permission::model()->hasSurveyPermission($this->sid,'surveycontent','delete' ))
        {
            ...
            $deleteUrl = Yii::app()->createUrl("admin/questions/sa/delete/surveyid/".$this->sid."/gid/".$this->gid."/qid/".$this->qid);
            $message = gT("Deleting  will also delete any answer options and subquestions it includes. Are you sure you want to continue?","js");
            $button .= '<a class="btn btn-default"  data-toggle="modal" title="'.gT("Delete").'" data-href="'.$deleteUrl.'" role="button" data-target="#confirmation-modal" data-message="'.$message.'">
                        <span class="text-danger glyphicon glyphicon-trash"></span>
                         </a>';
        }
