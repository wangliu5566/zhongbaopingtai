                ...
                else //THE FOLLOWING DEALS WITH SUBMITTING ANSWERS AND COMPLETING AN ACTIVE SURVEY
                {
                    ...
                    if (trim(str_replace(array('<p>','</p>'),'',$thissurvey['surveyls_endtext'])) == '')
                    {
                        if(!isset($_SESSION['saveActivityData']['code']) || !isset($_SESSION['saveActivityData']['activityId'])){
                            echo "<h1>数据有误,不能获取问答详情和用户信息,请联系管理员</h1>";exit;
                        }else{
                            $url = "http://crowdsourcing.jqweike.com/v1/frontend/activity/record-answer";
                            $post_data = array(
                                "code"=>$_SESSION['saveActivityData']['code'],
                                "activityId"=>$_SESSION['saveActivityData']['activityId'],
                            );
                            $ch = curl_init();
                           ...
                           if(!$output->data){
                               ...
                               if(!is_int($loginId)){
                                   echo "<h1>请重新提交,保存不成功</h1>";exit;
                               }

                           }
                        }
                        $completed = "<br /><input type='hidden' class='hidemenubutton'/><span>" . gT("Thank you!") . "</span><br /><br />\n\n"
                        . gT("Your survey responses have been recorded.") . "<br />请到活动页面领取积分和红包。<br />\n";
                    }
                    else
                    {
