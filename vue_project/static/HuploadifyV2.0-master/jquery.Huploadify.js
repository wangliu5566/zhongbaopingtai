		...
		this.each(function(index, element){
			...
			var uploadManager = {
				...
				init : function(){
					...
					inputStr += '</a><span style="margin-left:20px;">请上传小于800M，格式为.zip的压缩包</span>';
						...
						upload : function(fileIndex){
							if(fileIndex === '*'){
								// for(var i=0,len=uploadManager.filteredFiles.length;i<len;i++){
									uploadManager._uploadFile(uploadManager.filteredFiles[uploadManager.filteredFiles.length-1]);
								// }
							}else{
						...
						cancel : function(fileIndex){
							...
							if(fileIndex === '*'){
								...
								for(var i=len-1;i>=0;i--){
									uploadManager._deleteFile(uploadManager.filteredFiles[len-1]);
								}
								uploadManager._getInputBtn().val(''); //产品要求，做到删除过后再传同一个文件：清空input的值
					...
					_this.find('.uploadify-button').on('click',function(){
						// 在这里控制能否点击选择文件
						if(sessionStorage.againSlecte == 1){  //能否重新选择文件：1是不能，2是能重新选择
							message.showMessage("warning", "正在上传文件中，如果要重新选择，请暂停上传")
						}else{
				...
				_filter: function(files) {		//选择文件组的过滤方法
					...
					var typeArray = F.getFileTypes(option.fileTypeExts);
					 var typeArray = option.fileTypeExts;
					if(typeArray.length>0){
						for(var i=0,len=files.length;i<len;i++){
							var f = files[i];
							if(f.size<=option.fileSizeLimit){

								//自己在这里规定格式，在Apply页面添加各式，就是异常的慢
								if(f.name.indexOf('.zip')>0){
									// arr.push(f);
									arr[0] = f;
								}else{
									 message.showMessage('warning','文件格式不正确，请上传.zip格式！');
								}
							}else{
								 message.showMessage('warning','文件 "'+f.name+'" 大小超出800M限制！');
								continue;
							}
						}
					}
				...
				_renderFile : function(file){
					var $html = $(option.itemTemplate.replace(/\${fileID}/g,'fileupload_'+instanceNumber+'_'+1).replace(/\${fileName}/g,file.name).replace(/\${fileSize}/g,F.formatFileSize(file.size)).replace(/\${instanceID}/g,_this.attr('id')));
					//如果是非自动上传，显示上传按钮
					if(!option.auto){
					...
					if(option.breakPoints){
						...
						if(uploadedSize >= file.size){
							...
							initUppercent = Math.round(uploadedSize / file.size * 10000)/100 +'%';
						}

						$html.find('.uploadify-progress-bar').css('width',initWidth);
					}

					uploadManager._getFileList().children().remove();
					uploadManager._getFileList().append($html);

					//判断是否显示已上传文件大小
					if(option.showUploadedSize){
						var num = '<span class="progressnum"><span class="totalsize">${fileSize}</span></span>'.replace(/\${fileSize}/g,F.formatFileSize(file.size));
				...
				onProgress : function(fileblob, originalfile, loaded, total) {
					...
					// var percent = (loaded / originalfile.size * 100 + oldWidth).toFixed(4);
					var percent = (sessionStorage.uploadedSize / originalfile.size * 100).toFixed(4);
					var percentText = percent >= 100 ? '100%' : Math.floor(percent*100)/100 + '%';//校正四舍五入的计算误差
					// var percentText =  Math.round((sessionStorage.uploadedSize / originalfile.size * 10000))/100 +'%';
								...
								if(xhr.status == 200){
									if(option.breakPoints){
										...
										if(uploadedSize<=originalFile.size){
											...
											console.log('originalFile.size = ' + originalFile.size)
											uploadManager.uploadOver = false;
											if(!uploadManager.uploadStopped){
												file = originalFile.slice(uploadedSize,uploadedSize + option.fileSplitSize);
										  		file.status = originalFile.status;
